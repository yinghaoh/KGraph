 ...
 * the title based search feature.
 * 
 * @author aswin
 */
public class TitleSearchResource extends ServerResource {
	...
	/*
	 * Serves the title based search request
	 */
	@Get("json:javascript")
    public Representation searchArticles() {
    	...
    	if ( db != null ) {
    		...
    		logger.info("Request parameters: query = "+q+" type = "+type);
    		
    		try(Transaction tx = db.beginTx()) {
    			ResourceIterator<Node> nodes = GraphOperations.searchByTitle(db, type, q);
    			if(nodes == null) {
